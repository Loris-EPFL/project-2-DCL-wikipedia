...
# ----- NEW: build a set of main article IDs -----
main_ids = set()
for a in small:
    try:
        main_ids.add(int(a["id"]))
    except:
        pass

print(f"Main article count: {len(main_ids)}")

# ----- SEARCH USING MODERN QDRANT, BUT FILTER OUTPUT -----
...
with open(CSV_OUT, "w", encoding="utf-8", newline="") as f:
    ...
    for aid, vec in tqdm(vectors.items(), desc="Searching"):
        ...
        # Only compute similarities for MAIN articles
        if aid not in main_ids:
            continue

        results = client.search(
            collection_name=COLLECTION,
            query_vector=vec.tolist(),
            limit=TOP_K + 20,      # fetch more neighbors to avoid missing main ones
            with_payload=True
        )

        # Keep only neighbors that are MAIN articles
        neighbors_main = [r for r in results if r.id in main_ids and r.id != aid]

        # Take top K main-neighbors
        neighbors_main = neighbors_main[:TOP_K]

        for r in neighbors_main:
            writer.writerow([
                aid,
